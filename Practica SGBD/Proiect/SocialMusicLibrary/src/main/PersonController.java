...
public class PersonController {
    ...
    public MembersEntity findById(long id) {
        EntityManager em = emf.createEntityManager();
        Query query = em.createQuery("select t from MembersEntity t where t.idMember like :id");
        MembersEntity persons = (MembersEntity) query.setParameter("id", id).getSingleResult();
        em.close();
        return persons;
    }
    ...
    public List<FriendsEntity> getAllFriendsByEntity(MembersEntity membersEntity) {
        EntityManager em = emf.createEntityManager();
        Query query = em.createQuery("select t from FriendsEntity t where t.idMember1 like :id");
        List persons = query.setParameter("id", membersEntity.getIdMember()).getResultList();
        em.close();
        return persons;
    }
